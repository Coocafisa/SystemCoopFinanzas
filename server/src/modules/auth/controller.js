...
module.exports = function (dbInsert) {
  ...
  async function auth(req, res, data) {
    ...
    try {
      ...
      if (usuario?.sessionId !== "0" && validarInactividad) {
        return request.error(req, res, { message: "Ya tienes una sesión activa. Para continuar debes cerrar la anterior.", resetSession: usuario.sessionId}, 403);
      }
      ...
      if (updateAuth.affectedRows === 0) {
        return request.error(req, res, {message: "Error al actualizar la autenticación"}, 400);
      } 
  ...
  try {
    ...
    return request.error(
      req,
      res,
      {message:"Error al enviar el correo. Inténtalo de nuevo más tarde.", error},
      500
    );
  }
  }
  ...
  async function resetpass (req, res, data) {
    ...
    const refreshToken = async (req, res) => {
      ...
      try {
          ...
          if (updateAuth.affectedRows === 0) {
              return request.error(req, res, { message: 'Error al actualizar la sesión.' }, 500);
          }
